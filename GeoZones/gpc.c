...
void _stdcall gpc_free_polygon(gpc_polygon *p)
{
  int c;
  if(p->num_contours){ 
    for (c= 0; c < p->num_contours; c++)
...
void _stdcall gpc_del_contour(gpc_polygon *p,int n)
{
...
void _stdcall gpc_read_polygon(FILE *fp, int read_hole_flags, gpc_polygon *p)
{
  int c, v;
...
void _stdcall gpc_write_polygon(FILE *fp, int write_hole_flags, gpc_polygon *p)
{
  int c, v;
...
void _stdcall gpc_add_contour(gpc_polygon *p, gpc_vertex_list *new_contour, int hole)
{
  int             *extended_hole, c, v;
  gpc_vertex_list *extended_contour;
/*  FILE * fp;
...
void _stdcall gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
                      gpc_polygon *result)
{
  sb_tree       *sbtree= NULL;
...
void _stdcall gpc_free_tristrip(gpc_tristrip *t)
{
  int s;
...
void _stdcall gpc_polygon_to_tristrip(gpc_polygon *s, gpc_tristrip *t)
{
  gpc_polygon c;
...
void _stdcall gpc_tristrip_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
                       gpc_tristrip *result)
{
  sb_tree       *sbtree= NULL;
  it_node       *it= NULL, *intersect;
